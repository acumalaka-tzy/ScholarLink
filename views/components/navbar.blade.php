    ...
    <div class="px-4 sm:px-6 lg:px-10 py-3 sm:py-4">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ auth()->check() && auth()->user()->role === 'provider' ? route('provider.dashboard') : route('dashboard') }}" class="flex items-center gap-2 hover:opacity-80 transition">
            ...
            <div class="flex items-center gap-2 sm:gap-4">
                <a href="{{ auth()->check() && auth()->user()->role === 'provider' ? route('provider.dashboard') : route('dashboard') }}" class="px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition {{ request()->routeIs('dashboard', 'provider.dashboard') ? 'bg-blue-100 text-blue-600' : '' }}">
