    ...
    <div class="max-w-4xl mx-auto relative z-0">
        <div class="mb-12">
            ...
            <h1 class="text-5xl sm:text-6xl font-black text-gray-900 leading-tight tracking-tight">
                Tambah <span class="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Beasiswa</span>
            </h1>
            ...
            <form action="{{ route('provider.scholarships.store') }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nama_beasiswa" value="{{ old('nama_beasiswa') }}" placeholder="Masukkan nama beasiswa" class="w-full bg-gray-50 border-2 border-gray-200 rounded-2xl pl-14 pr-5 py-5 text-gray-900 font-bold focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 transition">
                    </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="5" placeholder="Tulis deskripsi lengkap beasiswa" class="w-full bg-gray-50 border-2 border-gray-200 rounded-2xl px-5 py-5 text-gray-900 font-bold resize-none focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 transition">{{ old('deskripsi') }}</textarea>
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <select name="provider_id" class="w-full bg-gray-50 border-2 border-gray-200 rounded-2xl px-5 py-5 text-gray-900 font-bold focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 transition">
                            @foreach($providers as $provider)
                                <option value="{{ $provider->id_provider }}">{{ $provider->nama_instansi }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-black mb-3 text-lg">Kategori</label>
                        <select name="category_id" class="w-full bg-gray-50 border-2 border-gray-200 rounded-2xl px-5 py-5 text-gray-900 font-bold focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 transition">
                ...
                <div>
                    ...
                    <input type="date" name="deadline" value="{{ old('deadline') }}" class="w-full bg-gray-50 border-2 border-gray-200 rounded-2xl px-5 py-5 text-gray-900 font-bold focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 transition">
                    @error('deadline') <p class="mt-2 text-red-500 font-bold text-sm">{{ $message }}</p> @enderror
                </div>

                <div class="pt-4 flex flex-col sm:flex-row gap-4">
                    <button type="submit" class="inline-flex items-center justify-center gap-3 bg-gradient-to-r from-blue-600 to-cyan-500 hover:scale-[1.02] transition px-8 py-5 rounded-2xl font-black text-white shadow-lg shadow-blue-500/20">
                        <i class="bi bi-save-fill"></i> Simpan Beasiswa
                    </button>
