...
<div class="w-full">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-blue-200 transition">
                        ...
                        @if($cvDoc)
                            <a href="{{ asset('storage/' . $cvDoc->file_path) }}" target="_blank" class="px-4 py-1.5 bg-white border border-gray-200 text-gray-700 hover:text-blue-600 hover:border-blue-200 text-xs font-black rounded-xl transition shadow-sm">
                                Buka
                            </a>
                        @else
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-blue-200 transition">
                        ...
                        @if($transkripDoc)
                            <a href="{{ asset('storage/' . $transkripDoc->file_path) }}" target="_blank" class="px-4 py-1.5 bg-white border border-gray-200 text-gray-700 hover:text-blue-600 hover:border-blue-200 text-xs font-black rounded-xl transition shadow-sm">
                                Buka
                            </a>
                        @else
                            <span class="text-xs font-bold text-gray-400 italic">Belum diupload</span>
            ...
            @else
                <div class="p-4 bg-gray-100 border border-gray-200 text-gray-500 font-bold text-sm text-center rounded-2xl">
                    Berkas lamaran ini sudah diproses dengan status final: <span class="font-black text-gray-800 uppercase">{{ $application->status }}</span>
                </div>
            @endif
...
<div id="modalWA" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white border border-gray-100 rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl transform scale-95 transition-transform duration-300">
        <div class="w-12 h-12 bg-emerald-50 text-emerald-500 rounded-2xl flex items-center justify-center text-xl mb-4 border border-emerald-100">
            💬
        </div>
        
        <h3 class="text-xl font-black text-gray-900 mb-2">Loloskan Berkas & Gabungkan Grup</h3>
        <p class="text-sm font-bold text-gray-500 mb-6 leading-relaxed">
            Mahasiswa ini akan dinyatakan **Lolos Tahap Berkas**. Masukkan tautan grup WhatsApp agar mahasiswa bisa masuk ke grup koordinasi seleksi berikutnya.
        </p>
        ...
        <form method="POST" action="{{ route('provider.applications.approve', $application->id_application) }}">
            ...
            <div class="mb-6">
                <label class="block text-xs font-black text-gray-500 uppercase mb-2 tracking-wide">Link WhatsApp Group</label>
                <div class="relative">
                    <input type="url" name="link_wa" required placeholder="https://chat.whatsapp.com/..." class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-gray-700 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition font-bold text-sm">
                </div>
            </div>

            <div class="flex gap-3 justify-end">
                <button type="button" onclick="toggleModalWA()" class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-2xl font-black transition text-sm">
                    Batal
                </button>
                <button type="submit" class="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-black transition text-sm shadow-lg shadow-emerald-500/20">
                    Konfirmasi Lolos
                </button>
            </div>
        </form>
