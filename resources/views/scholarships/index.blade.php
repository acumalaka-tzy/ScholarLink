...
@section('content')
<div class="min-h-screen bg-[#f4f7f9] py-10 px-4 md:px-10 overflow-hidden relative">
    ...
    <div class="max-w-7xl mx-auto relative z-0">
        <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-8 mb-14">
            <div>
                <div class="inline-flex items-center gap-3 bg-white border border-gray-200 rounded-full px-5 py-3 mb-6 shadow-sm">
                    ...
                    <span class="text-gray-700 text-sm font-black tracking-wide">ScholarLink Scholarship Portal</span>
                </div>
                <h1 class="text-5xl md:text-6xl font-black text-gray-900 leading-tight tracking-tight">
            ...
            @auth
                @if(auth()->user()->role == 'provider')
                    <a href="{{ route('provider.scholarships.create') }}" class="group relative inline-flex items-center justify-center gap-3 bg-gradient-to-r from-blue-600 to-cyan-500 hover:scale-[1.03] transition duration-300 px-8 py-5 rounded-2xl font-black text-white shadow-lg shadow-blue-500/20">
        ...
        @if($scholarships->isEmpty())
            <div class="bg-white border border-gray-100 rounded-[2.5rem] shadow-sm p-16 text-center">
                <div class="w-24 h-24 mx-auto rounded-[2rem] bg-gray-100 text-gray-400 flex items-center justify-center text-4xl mb-8">
                    <i class="bi bi-journal-x"></i>
                </div>
                <h2 class="text-3xl font-black text-gray-900 mb-3">Belum Ada Beasiswa</h2>
                <p class="text-gray-500 font-bold max-w-md mx-auto">Data scholarship belum tersedia saat ini. Silakan kembali lagi nanti.</p>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                @foreach($scholarships as $item)
                    <div class="group bg-white border border-gray-100 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-500 p-8 flex flex-col">
