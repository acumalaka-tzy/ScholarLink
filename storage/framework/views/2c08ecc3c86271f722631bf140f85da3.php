    ...
    <div class="max-w-7xl mx-auto relative z-0">
        ...
        <?php else: ?>
            <div id="scholarshipContainer" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                <?php $__currentLoopData = $scholarships; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div 
                        ...
                        data-name="<?php echo e($item->nama_beasiswa); ?>"
                        data-provider="<?php echo e($item->provider->nama_instansi ?? ''); ?>"
                        data-category="<?php echo e($item->category->nama_kategori ?? ''); ?>"
                        data-description="<?php echo e($item->deskripsi); ?>"
                    >
                        <div class="flex items-start justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                            <div class="flex flex-wrap gap-2">
                                <span class="bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-xs font-black border border-blue-100">
                                    <?php echo e($item->category->nama_kategori ?? 'General'); ?>
                        ...
                        <h3 class="text-2xl font-black text-gray-900 mb-4 group-hover:text-blue-600 transition"><?php echo e($item->nama_beasiswa); ?></h3>
                        <p class="text-gray-500 font-medium mb-8 flex-grow line-clamp-3"><?php echo e($item->deskripsi); ?></p>

                        <div class="space-y-3 mb-8">
                            <div class="flex items-center gap-3 text-sm font-bold text-gray-600">
                                <i class="bi bi-building text-blue-500"></i> <?php echo e($item->provider->nama_instansi ?? '-'); ?>

                            </div>
                            <div class="flex items-center gap-3 text-sm font-bold text-gray-600">
                                <i class="bi bi-calendar-event text-red-500"></i> <?php echo e(\Carbon\Carbon::parse($item->deadline)->format('d M Y')); ?>

                            </div>
                        </div>

                        <div class="flex gap-3 pt-4 border-t border-gray-100">
                            <a href="<?php echo e(route('scholarships.show', $item->id_beasiswa)); ?>" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-center py-3 rounded-xl font-black transition">Detail</a>
                            
                            <?php if(auth()->guard()->check()): ?>
                                <?php if(auth()->user()->role == 'mahasiswa'): ?>
                                    <form action="<?php echo e(route('applications.store')); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="id_beasiswa" value="<?php echo e($item->id_beasiswa); ?>">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wl\ScholarLink-main\ScholarLink-main\resources\views/scholarships/index.blade.php ENDPATH**/ ?>